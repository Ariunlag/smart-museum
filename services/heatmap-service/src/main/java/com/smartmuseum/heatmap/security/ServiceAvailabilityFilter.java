package com.smartmuseum.heatmap.security;
...
public class ServiceAvailabilityFilter extends OncePerRequestFilter {
    ...
    public ServiceAvailabilityFilter(
            ...
            @Value("${service.id:heatmap-service}") String serviceId,
    ...
    private boolean isServiceListedAsActive(String body) throws IOException {
        ...
        for (JsonNode node : root) {
            if (serviceId.equals(node.path("serviceId").asText())) {
                ...
                // Only block if explicitly DISABLED or REMOVED by an admin
                return !"DISABLED".equalsIgnoreCase(status) && !"REMOVED".equalsIgnoreCase(status);
            }
        }
        // Not found in list — public /services only returns ACTIVE services,
        // so absence means OFFLINE (temporary), not disabled. Fail open.
        return true;
    }
}
